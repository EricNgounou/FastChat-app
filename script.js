...
const account1 = {
  name: 'Yukio Taisei',
  password: 11111,
  picture: 'channel-pictures/my-channel.jpeg',
  friends: new Map([
    [
      'Rioto Yoshimassa',
      {
  ...
  Array.from(account.friends).forEach(friend => {
    ...
    <div class="chat-preview" data-name = "${friendAcc.name}">
        ...
        <div class="chat-preview-options">
          <div class="icon">
          <img class="markasread-icon" src="icons/markasread-icon.png">
          <p class="markasread">Mark as read</p>
          </div>          
          <div class="icon">
          <img class="delete-icon" src="icons/delete-icon.png">
  ...
  document.querySelector('.box').addEventListener(event, function (e) {
    if (event === 'click') {
      ...
      if (logOut) {
        ...
        loginMessage.style.color = 'rgba(3, 206, 233, 0.998);';
      }
    }
